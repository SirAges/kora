...
<RenderSelected item={selected} field={field}/>
            <Modal
                // transparent
            ...
            >
                ...
                <View className="px-2 py-2 flex-row items-center justify-between">
                    <MaterialCommunityIcons name="chevron-left" size={32} color={iconColor} onPress={() => setShowModal(false)}/>
                    <ThemedText className="font-semibold capitalize text-lg flex-1 text-center">{modal_tilte}</ThemedText>
                </View>
                    <FlatList
                        data={data}
                        renderItem={({ item }) => (
                            <RenderItem item={item} field={field} setShowModal={setShowModal}/>
                        )}
...
const RenderSelected=({item,field}:{item:any,field:string})=>{
if(field==='driver'){
    return <ProviderDetails user_id={item} onPress={undefined}  />;
}
  return (item&&
                <View
...
const RenderItem = ({ item, field,setShowModal }:{item:any,field:string,setShowModal:Dispatch<SetStateAction<boolean>>}) => {
    // console.log("item", item);
        const dispatch = useDispatch();
     
const onPress=()=>{
    dispatch(addToBooking({[field]:item}));
  setShowModal(false)
