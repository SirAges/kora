...
const RenderField = ({
    ...
    switch (fieldType) {
        case FormFieldType.INPUT:
            return <Input {...fieldProps} className="flex-1" />;
        case FormFieldType.PHONE_INPUT:
            return <PhoneInput {...fieldProps} className="flex-1" />;
        case FormFieldType.OTP_INPUT:
            return <OtpInput {...fieldProps} className="flex-1" />;
        case FormFieldType.SELECT:
            return <Select {...fieldProps} className="flex-1" />;
        case FormFieldType.SUGGESTION:
            return <Suggestion {...fieldProps} className="flex-1" />;
        case FormFieldType.SKELETON:
            return <Skeleton {...fieldProps} className="flex-1" />;
        case FormFieldType.TEXTAREA:
            return <TextArea {...fieldProps} />;
        case FormFieldType.CHECKBOX:
            return <Checkbox {...fieldProps} />;
        case FormFieldType.FILE:
            return <FilePicker {...fieldProps} className="flex-1" />;
        default:
            return null;
    }
};
...
const CustomFormField = ({
    ...
    const LIcon =
        LeftIcon && Icon[LeftIcon] ? Icon[LeftIcon] : Icon["Ionicons"];
    const RIcon =
        ...
        <Controller
            ...
            render={({ field, fieldState: { error } }) => (
                <View className="py-2 flex-1 px-1">
                    ...
                    <View
                        style={{
                            backgroundColor: [FormFieldType.CHECKBOX].includes(
                                fieldType
                            )
                                ? ""
                                : backgroundColor
                    ...
                    {/* Error Message */}
                    {error && (
                        <ThemedText className="text-red-500" type="error">
                            {error.message}
                        </ThemedText>
                    )}
                </View>
