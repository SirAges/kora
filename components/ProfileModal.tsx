    ...
    },
    {
      ...
      data: [
        ...
        },
        {
          ...
          label: "Start Date",
          value: new Date(user.subscription?.start_date).toLocaleDateString(),
        },
      ],
    },
    {
  ...
  return (
    <View className="items-center">
      ...
      {groupedData.map(({ title, data }, index) => (
        ...
        >
          <ThemedText className="font-bold text-lg mb-2">{title}</ThemedText>
          {data.map(
            ({ key, label, value }) =>
              value !== undefined && (
                <View
    ...
    const getTotalCacheSize = async () => {
      try {
        ...
        let reduxSize = items.reduce(
          (acc, [, value]) => acc + (value ? new Blob([value]).size : 0),
          0
        );
        ...
        setCache(((reduxSize + fileSize) / (1024 * 1024)).toFixed(2) + "MB");
      } catch (error) {
    ...
    <View>
      <View className="px-2 py-2 rounded-md border border-gray-200 my-3">
        ...
        <RenderItem
          ...
          onPress={() => onItemPress("Currency", "currency", currencies)}
        />
        <RenderItem
          title="Units"
        ...
        <RenderItem
          ...
          onPress={() =>
            onItemPress("Temperature Scale", "temperature", temperatures)
          }
        />
        <RenderItem
          ...
          onPress={() =>
            onItemPress("First Day of Week", "fdow", firstDayOfWeekOptions)
          }
        />
        <RenderItem
          title="Date Format"
          field={user?.setting?.date_format}
          onPress={() => onItemPress("Date Format", "date_format", dateFormats)}
        />
        <RenderItem
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              ...
              >
                ...
                <MaterialCommunityIcons
                  name={
                    item.value === user?.setting?.[selectedItem?.field]
                      ? "radiobox-marked"
                      : "radiobox-blank"
                  }
                  size={20}
        ...
        renderItem={({ item }) => (
          <View className="flex-row items-center justify-between">
            ...
            <Switch
              ...
              thumbColor={
                user?.notification_preferences[item?.value] ? iconColor : color
              }
              ios_backgroundColor={card}
              onValueChange={(value) => onSwitchChange(value, item?.value)}
...
const RenderSecurity = ({ user }) => {
  ...
  return (
    <View className="rounded-md border border-gray-200 px-2 py-2 my-3">
          ...
          <View className=" my-2">
            ...
            >
              ...
              <Switch
                ...
                thumbColor={
                  user?.security_preferences[item?.value] ? iconColor : color
                }
                ios_backgroundColor={card}
                onValueChange={(value) => onSwitchChange(value, item?.value)}
      ...
      <View className="space-y-2">
        ...
        <View>
          {user?.connected_devices.map(
            ({ device_id, brand, model_name, device_type, device_name }) => (
              <View
              ...
              >
                <View className="flex-row items-center justify-between py-1">
                  ...
                  <MaterialCommunityIcons
                    onPress={() => onRemoveDevicePress(device_id)}
                    name="trash-can"
                    size={20}
                    color={danger}
                  />
                ...
                >
                  <ThemedText className="text-xs">{brand}</ThemedText>
                  <ThemedText className="text-xs">{model_name}</ThemedText>
                </View>
              </View>
            )
          )}
      ...
      <TouchableOpacity
        onPress={() =>
          onActionsPress("deactivate_account", !user?.deactivate_account)
        }
        style={{
      ...
      <TouchableOpacity
        onPress={() => onActionsPress("delete_account", !user?.delete_account)}
        style={{
      ...
      >
        <ThemedText className="font-semibold">
          {user?.delete_account ? "Delete Account" : "Cancel Account Deletion"}
        </ThemedText>
        <ThemedText
          type="subtitle"
          className="text-sm"
